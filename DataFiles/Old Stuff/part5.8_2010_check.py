...
for row in range(0,len(monthlyDF)):
    ...
    for row2 in range(0,len(thisMonthsDF)):
        ...
        for row3 in range(0,len(thisMonthsCountyDF)):
            ...
            # Doing nyc map right now, if the county is westchester, choose that index
            target = 0
            if thisCountyStr = "Westchester County":
                target = 1
            
            ####################################################################### ABOVDE THIS CODE IS TESTED #########################
            mapArray[target]
            ...
            if (len(thisMonthsCountyGeo_nycGDF) == 0) and thisCountyStr != "Westchester County":                                                                                  # If these r no reports for this month-year so skip
                # print("----------------- "+ thisMonthYrStr +" NO GEOID THIS MONTH FOR COUNTY1 "+thisCountyStr)#+thisMonthsCountyGeo_nycGDF[GDF_NYC_GEOID_COL][row3])
                continue
            if (len(thisMonthsCountyGeo_westGDF) == 0) and thisCountyStr == "Westchester County":                                                                                  # If these r no reports for this month-year so skip
                # print("----------------- "+ thisMonthYrStr +" NO GEOID THIS MONTH FOR COUNTY2 "+thisCountyStr)#+thisMonthsCountyGeo_nycGDF[GDF_NYC_GEOID_COL][row3])
                continue
